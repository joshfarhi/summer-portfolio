import type { Metadata } from "next";
import Link from "next/link";

export const metadata: Metadata = {
  title: "Digital Nomad: Vietnam | My Experience",
  description: "Placeholder content for an upcoming post about traveling in Vietnam.",
};

export default function Page() {
  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl">
        <div className="mb-8">
          <Link href="/blog" className="text-muted-foreground hover:text-foreground">
            &larr; Back to Blog
          </Link>
        </div>

        <h1 className="text-4xl font-bold mb-4">Vietnam Travel Notes (Placeholder)</h1>
        <div className="inline-block bg-orange-500/20 text-orange-500 px-3 py-1 rounded-full text-sm font-medium mb-6">
          Currently writing
        </div>

        <p className="text-muted-foreground leading-relaxed">
          This is placeholder content for the upcoming post about traveling in Vietnam. Expect sections on Hanoi, Ho Chi Minh City, cuisine, and train routes.
        </p>
      </div>
    </div>
  );
}


